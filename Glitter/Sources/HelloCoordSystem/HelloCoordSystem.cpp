//
// Created by HFY on 2019/2/13.
//

#include "HelloCoordSystem.h"
#include "glitter.hpp"
...
int HelloCoordSystem::runHelloCoordSystem() {
    ...
    while (glfwWindowShouldClose(mWindow) == false) {
        ...
        view = glm::translate(view, glm::vec3(1.0f, 0.0f, -3.0f));// view matrix 注意，我们将矩阵向我们要进行移动场景的反方向移动。
...
int HelloCoordSystem::runHelloCoordSystemBox3D() {
    // Load GLFW and Create a Window
    glfwInit();
    glfwWindowHint(GLFW_CONTEXT_VERSION_MAJOR, 4);
    glfwWindowHint(GLFW_CONTEXT_VERSION_MINOR, 0);
    ...
    while (glfwWindowShouldClose(mWindow) == false) {
        ...
        glm::mat4 view = glm::mat4(1.0f);
        glm::mat4 projection = glm::mat4(1.0f);
        model = glm::rotate(model,  (float)glfwGetTime() * glm::radians(-55.5f), glm::vec3(0.5, 1.0, 0.0));
        view = glm::translate(view, glm::vec3(0.0f, 0.0f, -3.0f));// view matrix 注意，我们将矩阵向我们要进行移动场景的反方向移动。
        projection = glm::perspective(glm::radians(45.0f), (float)mWidth / (float)mHeight, 0.1f, 100.0f);
...
int HelloCoordSystem::runHelloCoordSystemMoreBox3D()  {
    // Load GLFW and Create a Window
    glfwInit();
    glfwWindowHint(GLFW_CONTEXT_VERSION_MAJOR, 4);
    glfwWindowHint(GLFW_CONTEXT_VERSION_MINOR, 0);
